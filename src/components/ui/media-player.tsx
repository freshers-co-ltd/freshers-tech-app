	...
	return (
		<MediaPlayerContext.Provider value={contextValue}>
			<RootPrimitive
				...
				onKeyUp={onKeyUp}
				className={cn(
					'relative isolate flex flex-col overflow-hidden bg-background outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 data-disabled:pointer-events-none data-disabled:opacity-50 [&_video]:relative [&_video]:object-contain',
		...
		<VolumeIndicatorPrimitive
			...
			<div className="fade-in-0 zoom-in-95 flex animate-in flex-col items-center gap-3 rounded-lg bg-black/30 px-6 py-4 text-white backdrop-blur-xs duration-200">
				...
				<div className="flex items-center gap-1">
					{['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j'].map((key, idx) => (
						<div
							key={`vb-${key}`}
							className={cn(
								'w-1.5 rounded-full transition-all duration-150',
								idx < activeBarCount && !mediaMuted ? 'scale-100 bg-white' : 'scale-90 bg-white/30',
							)}
							style={{
								height: `${12 + idx * 2}px`,
	...
	return (
		<MediaPlayerTooltip tooltip={mediaPaused ? 'Play' : 'Pause'} shortcut="Space">
			<Button
				...
				variant="ghost"
				size="icon"
				className={cn("size-8 text-white [&_svg:not([class*='fill-'])]:fill-current", className)}
				onClick={onPlayToggle}>
	...
	return (
		<MediaPlayerTooltip
			...
			<Button
				...
				variant="ghost"
				size="icon"
				className={cn('size-8 text-white', className)}
		...
		return chapterCues.slice(1).map((chapterCue, index) => {
			...
			return (
				<div
					...
					className="absolute top-0 h-full bg-zinc-50"
					style={{
						width: '.1563rem',
						left: `${position}%`,
			...
			<SliderPrimitive.Root
				...
				<SliderPrimitive.Track className="relative h-1 w-full grow overflow-hidden rounded-full bg-background/40">
					<div
						...
						className="absolute h-full bg-background/70 will-change-[width]"
						style={{
							width: `${bufferedProgress * 100}%`,
						}}
					/>
					<SliderPrimitive.Range className="absolute h-full bg-background will-change-[width]" />
					{seekState.isHovering && seekableEnd > 0 && (
						<div
							data-slot="media-player-seek-hover-range"
							className="absolute h-full bg-background/70 will-change-[width,opacity]"
							style={{
				...
				<SliderPrimitive.Thumb className="relative z-10 block size-2.5 shrink-0 rounded-full bg-background shadow-sm ring-ring/50 transition-[color,box-shadow] will-change-transform hover:ring-4 focus-visible:outline-hidden focus-visible:ring-4 disabled:pointer-events-none disabled:opacity-50" />
			</SliderPrimitive.Root>
	...
	return (
		<div
			...
			<SliderPrimitive.Root
				...
				<SliderPrimitive.Thumb className="block size-2.5 shrink-0 rounded-full bg-background shadow-sm ring-ring/50 transition-[color,box-shadow] will-change-transform hover:ring-4 focus-visible:outline-hidden focus-visible:ring-4 disabled:pointer-events-none disabled:opacity-50" />
			</SliderPrimitive.Root>
		</div>
	);
}
...
function MediaPlayerTime(props: MediaPlayerTimeProps) {
	...
	return (
		<TimePrimitive
			...
			dir={context.dir}
			{...timeProps}
			className={cn('flex items-center gap-1 text-white text-sm', className)}>
			<span className="tabular-nums">{times.current}</span>
			<hr className="w-px h-4 mx-1 bg-current/50" />
	...
	return (
		<DropdownMenu modal={modal} open={open} defaultOpen={defaultOpen} onOpenChange={onOpenChange}>
			<MediaPlayerTooltip tooltip="Playback speed" shortcut={['<', '>']}>
				<DropdownMenuTrigger asChild>
					<Button
						...
						variant="ghost"
						size="icon"
						className={cn('h-8 w-16 text-white aria-expanded:bg-accent/50', className)}>
						{mediaPlaybackRate}x
					</Button>
				</DropdownMenuTrigger>
			</MediaPlayerTooltip>
			<DropdownMenuContent
				...
				align="center"
				className="min-w-(--radix-dropdown-menu-trigger-width) data-[side=top]:mb-3.5 bg-white text-foreground shadow-lg border">
				{speeds.map((speed) => (
					<DropdownMenuItem
						key={speed}
	...
	return (
		<DropdownMenu modal={modal} open={open} defaultOpen={defaultOpen} onOpenChange={onOpenChange}>
			...
			<DropdownMenuContent
				...
				sideOffset={sideOffset}
				container={context.portalContainer}
				className="w-56 data-[side=top]:mb-3.5 bg-white text-foreground shadow-lg border">
				...
				<DropdownMenuSub>
					...
					<DropdownMenuSubContent className="bg-white text-foreground shadow-lg border">
						{speeds.map((speed) => (
							<DropdownMenuItem
								key={speed}
								className="justify-between"
	...
	return (
		<Tooltip {...tooltipProps} delayDuration={tooltipDelayDuration}>
			...
			<TooltipContent
				...
				className="flex items-center gap-2 border bg-white text-foreground px-2 py-1 font-medium data-[side=top]:mb-3.5 shadow-lg [&>span]:hidden">
				<p>{tooltip}</p>
