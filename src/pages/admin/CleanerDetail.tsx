			...
			<div className="grid gap-4 md:grid-cols-2 mb-6">
				<Card className="p-4">
					<div className="flex items-center gap-3">
						...
						<div className="flex-1 min-w-0">
							<p className="font-semibold truncate">{cleaner.full_name || 'Unknown'}</p>
							<p className="text-sm text-muted-foreground truncate">{cleaner.email}</p>
							<div className="flex items-center gap-2 mt-1">
								<span
									className={`px-2 py-0.5 rounded-full text-xs font-medium ${
										cleaner.banned_until ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
									}`}>
									{cleaner.banned_until ? 'Banned' : 'Active'}
								</span>
								{cleaner.is_verified && (
				...
				<Card className="p-4">
					<div className="grid grid-cols-4 gap-3 text-center">
						...
						<div>
							<p className="text-xl font-bold">£{stats?.total_earnings?.toFixed(0) || '0'}</p>
							...
							<button
								...
								<div className="flex-1 min-w-0">
									...
									<p className="text-xs text-muted-foreground">
										...
										{cleaning.host_name || 'Unknown Host'}
									</p>
								</div>
