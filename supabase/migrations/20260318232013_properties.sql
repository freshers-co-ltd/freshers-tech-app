...
CREATE POLICY "Authorised users can insert properties"
...
CREATE POLICY "Authorised users can update properties"
ON public.properties
FOR UPDATE
TO authenticated
USING (
...
CREATE POLICY "Authorised users can manage property media"
ON storage.objects
FOR ALL
TO authenticated
USING (
...
CREATE OR REPLACE FUNCTION public.enforce_property_immutability()
RETURNS TRIGGER
SECURITY DEFINER
...
CREATE TRIGGER trigger_property_immutability
    BEFORE UPDATE ON public.properties
    FOR EACH ROW
...
GRANT EXECUTE ON FUNCTION public.soft_delete_property TO authenticated;