...
RETURNS TABLE (hourly_rate NUMERIC, target_times JSONB, updated_at TIMESTAMPTZ)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  RETURN QUERY SELECT c.hourly_rate, c.target_times, c.updated_at FROM cleaner_pay_config c WHERE c.id = 1;
END;
$$;
